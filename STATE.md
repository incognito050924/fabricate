...
| # | 무엇 | 왜 남겼나 |
| --- | --- | --- |
| **D-4** | `close` 가 성공해도 `.fabricate/sessions/<id>/active` 가 안 지워져 **이후 모든 턴을 `Stop` 훅이 막는다.** 인터뷰를 끝내면 일반 대화가 불가능해진다 | 술어 여섯에 없다. 다음 세션이 먼저 볼 것 — 지금은 표식을 손으로 지워야 풀린다 |
| **D-5** | 겹침 검사가 한 낱말 답변에서 오작동한다(`src/ledger.ts`, 임계 0.8) | 술어 여섯에 없다 |
| — | `준비도: contradictions=0 unsure=0 demoted=0 ready=true` 는 그대로다 | 영어를 옮긴 말이 아니라 영어 식별자라 목표 5 의 범위 밖이고, `IP-2g` 가 그 문자열에 걸려 있다. 사용자에게 안 읽히는 줄인 것은 맞다 |
