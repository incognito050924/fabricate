...
## 완료 기준 여섯을 구현했다 (2026-07-29)

앞 절이 확정한 완료 기준 여섯을 전부 만들었다. 잠긴 기록 `b63faf03-2aa0-4cb1-9be6-25107cc83a1e`
의 목표 술어가 완료 정의였고, `fabricate check <id>` 가 **exit 0** 이다.

```
bun run verify                        59/60 초록 (축 A 43/43 · 축 B 13/14 · 축 C 3/3)
fabricate check b63faf03-…            6개 목표가 충족됐습니다, exit 0
```

축 A 는 43/43 그대로다. 남은 하나는 세션 A 이래의 `S-12` 이고 이번 변경과 무관하다.
**lint 가 다시 초록이 됐다** — 인터뷰를 자기 저장소에서 돌리면서 생긴 `.fabricate/` 런타임
데이터를 biome 가 검사하고 있었다. `.gitignore` 는 그것을 무시하는데 biome 는 아니었다.

### 무엇을 만들었나 — 목표 술어별

| 목표 | 만든 것 | 거부가 어디서 서는가 |
| --- | --- | --- |
| 0 | `question` 에 `--recommend`(추천 답변)·`--because`(그 근거) **필수** | 빈손 질문은 장부에 안 남는다 |
| 1 | 새 기록 종류 **`remark`** — 어느 질문에도 안 묶이는 사용자 발화를 원문 그대로 받는다. `--overturns` 를 붙이면 그 전제에 기대던 쟁점이 다시 열린다 | `answer` 는 질문 없이 못 들어가고 `remark` 는 들어간다 |
| 2 | 완료 판정 기준이 **0건이면 `close` 거부** | `완료 판정 기준이 없습니다.` |
| 3 | `turn record` 가 성공할 때마다 **상태 블록** 출력 — 확정된 것 / 아직 안 정해진 것 / 지금 이해하고 있는 뜻 (`src/status.ts`) | 세 칸이 실제 장부를 읽는지 대칭으로 검사한다 |
| 4 | 스킬 설명이 *"쓴다"* → *"권한다 — 에이전트가 스스로 시작하지 않는다"*. 본문에 비용 실측과 권하는 문안 | 모델 경로로 열린 세션은 원문이 없어 `close` 가 거부(기존 IP-2ⓜ) |
| 5 | 배송물 전체에서 맹검·되말·재진술·차원·드라이버·하네스·시드·사다리 타기·미커버·라우팅 제거. 닫기 거부 사유도 전부 새 문안 | 배송물을 훑는 검사가 하나라도 남으면 빨개진다 |

검사는 `verify/fixtures/G-0` ~ `G-5.test.ts` 여섯. 목표마다 RED 를 먼저 관측하고 구현했다.
**축 A(`verify/criteria.ts`)에는 넣지 않았다** — 축 A 는 `GOAL.md` §2 의 통합 술어이고
이 여섯은 잠긴 기록의 술어다. 완료 정의 둘을 한 칸에 섞지 않는다(§4-4).

### 고른 길 — 왜 훅으로 강제하지 않았나

목표 3 을 `Stop` 훅으로 강제하려 했다. 상태를 안 낸 턴을 막는 것이다. **접었다** —
`GOAL.md` §2 IP-2ⓛ 이 *"정상 진행 턴(장부가 늘었음)은 막지 **않는다**"* 로 비준돼 있어서
정면 충돌하고, 그건 §7-4(술어를 약화시키지 않고는 진행 불가)라 사용자 결정 사안이다.
그래서 **모든 기록이 상태를 자동으로 내는** 쪽으로 갔다. 기계가 보는 것은 블록이 만들어졌는가
까지이고, 그것을 사용자에게 실제로 붙여 보이는 것은 프롬프트에 남는다 — **알면서 남기는 잔여다.**

D-1 의 교훈("CLI 에 기능이 있어도 `SKILL.md` 가 안 시키면 죽는다")은 여섯 전부에서 피했다.
`--recommend`·`remark`·`criterion`·상태 블록 모두 설명서에 지시가 있고, 그 지시의 존재
자체를 `G-*` 검사가 본다.

### 남은 결함 셋도 같이 고쳤다 — 술어 목록을 방패로 쓰지 않는다

처음에는 D-3·D-4·D-5 를 *"목표 술어에 없다"* 로 남겼다. **사용자가 그것을 회피라고 짚었고,
그게 맞다.** 셋 다 `deep-interview` 코드이고, 셋 다 **그 인터뷰가 자기 자신을 돌리다 걸린
것**이다. 술어 여섯은 사용자가 그 자리에서 말로 옮긴 것이지, 고칠 것의 전부가 아니었다.

| # | 무엇이 문제였나 | 어떻게 고쳤나 |
| --- | --- | --- |
| **D-3** | `challenge` 가 `--answer` 를 필수로 요구해 **질문의 전제**는 칠 수 없었다. 그래서 이미 구현된 기능을 *"만들까요"* 로 묻는 질문이 검토를 통과해 사용자에게 갔다 — 대화를 못 본 검토자는 문안을 재지 사실을 재지 않는다 | `--answer` 를 선택으로 내린다. 질문만 걸어도 기록된다. `--citation`(실재 파일:줄)은 그대로 필수. `SKILL.md` 가 "보내기 전에 전제를 코드에 대본다"를 지시한다 |
| **D-4** | `close` 가 성공해도 `active` 표식이 남아 **그 뒤 모든 턴을 `Stop` 훅이 막았다.** 인터뷰를 끝내면 일반 대화가 불가능해진다 | 닫기가 통과할 때 표식을 지운다. **거부된 닫기는 안 지운다** — 인터뷰가 아직 진행 중이다 |
| **D-5** | 겹침 검사가 한 낱말 답변에서 오작동. `(가)` 면 `1/1 = 1.0` 이라 그 낱말이 든 문장은 전부 거부됐다. 에이전트가 배우는 것은 더 생각하기가 아니라 **그 글자를 피해 쓰기**다 | 규칙을 둘로 나눈다. ① 보탠 말이 하나도 없으면 길이와 무관하게 거부 ② 비율 규칙은 답변이 **다섯 낱말 이상**일 때만 — 그 아래에서 비율은 아무것도 재지 못한다 |

**준비도 줄도 고쳤다.** `준비도: contradictions=0 unsure=0 …` 을 "영어 식별자라 목표 5 범위
밖"으로 넘겼었는데, 사용자가 읽고 아무것도 알 수 없는 줄인 것은 사실이었다. 이제
`준비도: 안 풀린 어긋남 contradictions=0 · 확신 못 한 답 unsure=0 · 근거 없이 닫은 것
demoted=0 · 닫아도 되는가 ready=true` 로 나온다. `key=value` 를 남겨 `IP-2g` 를 깨지 않는다.

**이로써 D-1~D-6 여섯이 전부 닫혔다.** 검사는 `D-3`·`D-4`·`D-5.test.ts` 여덟과
`G-5` 에 더한 준비도 검사.

### `verify` 가 사용자 홈에 항목을 남기고 있었다 — 죽으면 `finally` 가 안 돈다

사용자 홈에 `fabricate-verify-*` 마켓플레이스 둘과 캐시 디렉터리 둘이 남아 있었다.
`GOAL.md` §4-9 가 `verify` 의 책임 셋 중 하나로 *"끝난 뒤 자기가 등록한 항목을 지운다"* 를
못박은 자리다.

**teardown 코드는 틀리지 않았다. 도달할 수 없었을 뿐이다.**
`verify/lib/host-session.ts` 의 정리는 `finally` 안에 있고, `finally` 는 프로세스가 죽으면
안 돈다. `bun run verify` 를 중간에 죽이는 것은 특별한 일이 아니다 — 시간 초과, Ctrl-C,
작업 취소. 남은 둘이 그 증거였다(21:39 죽인 구동 · 22:24 시간 초과로 죽은 구동, 둘 다
`d7e16d6`). **정상 종료한 구동은 아무것도 안 남겼다** — 그래서 결론은 "정리가 틀렸다"가
아니라 **"정리가 도달 불가능한 경우가 있다"** 다.

**신호 처리기로는 못 막는다 — `SIGKILL` 은 잡을 수 없다.** 잡을 수 있는 것은 **다음 구동
시작 때 쓸어내는 것**이고, 잔여물은 애매함 없이 식별된다: **소스 디렉터리가 사라져 있으면**
죽은 구동의 것이고, **아직 있으면 지금 돌고 있는** 구동의 것이라 건드리면 안 된다.

`verify/lib/stale-marketplaces.ts` 를 만들어 `verify/run.ts` 시작점에 걸었다. 실측:

```
등록  fabricate-verify-sweeptest-…(소스 지움) + fabricate-verify-live-…(소스 살림)
쓸기  { "removed": ["fabricate-verify-sweeptest-…"], "failed": [] }
결과  죽은 캐시 디렉터리까지 지워짐 · 살아 있는 항목은 그대로
```

실측 하나 더: **`claude plugin marketplace remove` 는 등록부와 설치 기록은 지우지만 캐시
디렉터리는 남긴다.** 그래서 쓸기가 `cache/<name>` 을 따로 지운다.

**조용히 지우지 않는다.** 지운 것과 못 지운 것을 `verify` 출력 맨 위에 적는다(§4-7).
쓸 것이 없으면 한 줄도 안 나온다.

**남는 한계**: 청소는 다음 구동 시작 때 일어난다. 죽은 직후부터 다음 구동 전까지는 항목이
사용자 홈에 남아 있다. `SIGKILL` 을 잡을 수 없으므로 그보다 이른 시점은 없다. 검사
다섯(`verify/fixtures/teardown.test.ts`)은 **선별 규칙만** 본다 — 사용자 홈을 건드리지 않고
판정할 수 있는 범위가 거기까지다. `S-12` 는 그대로 `n/a` 다(§4-9 가 적용 범위를 그렇게 그었다).

### 그래도 안 하는 것 하나 — 이건 미룬 게 아니라 결론이다

**목표 3 을 `Stop` 훅으로 강제하지 않는다.** `GOAL.md` §2 IP-2ⓛ 과 충돌해서만이 아니라,
**개정해도 얻는 것이 없어서다.** 훅이 볼 수 있는 것은 "상태 블록을 만들었는가"까지이고
그것은 이미 모든 기록이 자동으로 낸다. **"사용자가 실제로 봤는가"는 훅이 모델의 응답 내용을
못 보므로 어떤 훅으로도 못 잰다** — §8 이 잔여로 선언한 부류 그대로다.
...
| 2026-07-29 | **완료 기준 여섯을 구현했다.** `fabricate check b63faf03-…` **exit 0** · `bun run verify` **59/60**(축 A 43/43 유지, 남은 하나는 `S-12`). 질문에 추천·근거 필수 · 사용자가 먼저 꺼낸 말을 담는 `remark` · 완료 판정 기준 없으면 닫기 거부 · 매 기록마다 상태 블록 자동 출력 · 자동 시작 안 하고 권한다 · 직역·음차 용어 제거. 검사 `G-0`~`G-5` 여섯을 새로 썼고 **축 A 에는 안 넣었다**(완료 정의 둘을 안 섞는다). 덤으로 **lint 복구** — 자기 저장소에서 인터뷰를 돌리며 생긴 `.fabricate/` 를 biome 가 검사하고 있었다. **목표 3 의 훅 강제는 접었다** — IP-2ⓛ 과 정면 충돌이라 §7-4 다. **D-4·D-5 는 안 고쳤다**(술어 밖). 전문은 "완료 기준 여섯을 구현했다" 절 |
| 2026-07-29 | **남은 결함 셋도 닫았다 — D-1~D-6 전부.** 술어에 없다고 미룬 D-3·D-4·D-5 를 사용자가 회피라고 짚었고 그게 맞았다. **D-3** `challenge` 의 `--answer` 를 선택으로 내려 **질문의 전제**를 코드로 칠 수 있게 했다(인용은 그대로 필수) · **D-4** 닫기가 통과하면 활성 표식을 지운다, 거부된 닫기는 안 지운다 · **D-5** 겹침 규칙을 둘로 — 보탠 말 0이면 무조건 거부, 비율은 답변 다섯 낱말 이상일 때만. 준비도 줄도 한국어 이름표를 달았다(`key=value` 는 남겨 `IP-2g` 유지). `bun run verify` **59/60** 유지 · `check` exit 0. **목표 3 의 훅 강제는 안 한다 — 미룬 게 아니라 결론이다**: 훅은 모델의 응답 내용을 못 보므로 "사용자가 봤는가"를 어떤 훅으로도 못 잰다 |
| 2026-07-29 | **`verify` 가 사용자 홈에 남긴 잔여물을 치우고 원인을 고쳤다.** `fabricate-verify-*` 마켓플레이스 둘 + 캐시 둘이 남아 있었다(§4-9 위반). **teardown 코드는 틀리지 않았고 `finally` 가 프로세스 사망 시 안 도는 것이 원인이었다** — 죽인 구동과 시간 초과로 죽은 구동이 각각 하나씩 남겼고 정상 종료한 구동은 아무것도 안 남겼다. `SIGKILL` 은 못 잡으므로 **다음 구동 시작 때 쓸어내는** 것으로 고쳤다(`verify/lib/stale-marketplaces.ts`). 판별은 **소스 디렉터리가 사라졌는가** — 살아 있으면 지금 돌고 있는 구동이라 건드리지 않는다. 실측으로 죽은 것만 지우고 산 것은 남기는 것을 확인했다. 새 실측: **`claude plugin marketplace remove` 는 등록부·설치 기록은 지우지만 캐시 디렉터리는 남긴다.** 지운 것·못 지운 것을 출력 맨 위에 적는다(§4-7). 한계 — 청소는 다음 구동 시작 때이고 그보다 이른 시점은 없다 |
