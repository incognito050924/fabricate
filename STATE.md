...
| 있음 | 있음 | 일부 초록 (**지금 여기다 — 37/60**) | **세션 B** (진행 중) |
...
**한 줄 상태**: 목표 비준(개정 5) · 세션 A 완료 · **세션 B 진행 중(2026-07-29)** — 걷는 경로 위에 게이트를 얹는 중이다. `bun run verify` → **37/60 초록, exit 1** (축 A 22/43 · 축 B 12/14 · 축 C 3/3). 브랜치는 `bootstrap/walk`. 조각별 진척은 아래 **"세션 B 진행"** 절이 정본이다.
...
닫는 명령: **`bun run verify` 하나.** 구현됨 — 지금 **37/60 초록, exit 1**.
...
## 세션 B 진행 — 조각마다 여기에 적는다 (2026-07-29)

세션 B는 끝까지 가는 단일 구동이다. 컨텍스트는 요약되지만 디스크는 남는다.
**조각 하나가 끝날 때마다 `bun run verify` 를 돌리고 이 절에 결과를 적고 커밋한다.**
...
| 3 | **IP-1 ⓑ~ⓗ** — fixture 일곱, `src/` 변경 0 | **37/60** (A 22/43 · B 12/14 · C 3/3) | (이 커밋) |
