...
`fabricate turn record`는 성공할 때마다 **지금까지의 상태 블록**을 출력한다. 세 칸이다.
...
**턴을 끝내기 전, 그 턴의 마지막 기록에서 나온 블록을 사용자 응답에 그대로 붙인다.**
사용자가 요청해서 보이는 것이 아니다 — 매번 보인다. 사용자가 "지금 어디까지 왔지"를
물어야 알 수 있다면, 당신이 뜻을 어디서 틀었는지도 사용자는 물어야만 알게 된다.
