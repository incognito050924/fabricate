...
3. **`src/interview/finalize.ts` — 게이트를 기록 경로 앞으로 옮긴다(X2).** `finalizeIntent`의 검사를
   ...
   → **[막혀 있다 — 사용자 판정 사안. 아래 "0단계 3번 차단" 참조.]** ac-1 문안의 라운드-0 선행
   게이트와 `acceptance/ac-3.test.ts`의 단언이 **양립 불가**여서, 게이트를 실기록 경로에 합치는
   순간 ac-3이 빨개진다. 착수 전에 사용자가 선택지를 골라야 한다.
...
#### 0단계 3번 차단 — **사용자 판정 사안** (ac-1 문안 ↔ ac-3 동결 테스트 충돌)

**착수 전에 사용자가 골라야 한다. 에이전트가 임의로 정하지 마라.**

- **충돌 내용**: `contract/criteria.md:7` · `contract/contract-draft.md:72` ·
  ...
  **`createTurnLog()`(= goal_state 없음)에 `recordFiredTurn`을 넣어 `recorded === true`**를
  단언한다. **양립 불가다.** 게이트를 실기록 경로에 합치는 순간 **ac-3이 빨개진다.**
  `acceptance/`는 고칠 수 없다(§5).
- **단서**: `acceptance/ac-10f.test.ts:305-315`도 같은 단언을 갖지만 **모듈 부재로 오늘
  ...
  **vacuous-pass path**로 지목한 자리다(검사를 가진 `finalizeIntent`는 호출자 0, 실제로
  기록하는 경로에는 검사가 없다).
- **선택지 셋**
  1. **게이트를 조건부로 적용한다** — 예: goal_state가 있을 때만 순서/참조를 본다.
     ac-3은 초록으로 남지만 **ac-1 절1의 보증이 좁아진다**(문안이 요구한 fail-closed가
     "목표 상태가 있을 때만"으로 축소).
  2. **기록 경로를 분리한다** — 게이트 있는 경로와 없는 경로를 따로 둔다. 동결 테스트는
     전부 초록이지만 **X1(모듈 섬)이 악화된다** — 실기록 경로가 또 하나 게이트 밖에 선다.
  3. **미검증으로 남기고 사용자에게 드러낸다** — 코드를 고치지 않고 §7에 "ac-1 절1의
     라운드-0 선행 게이트는 실기록 경로에 없다, 동결 테스트가 그것을 금지한다"로 적는다.
     정직하지만 X2가 그대로 남는다.
- **G3 — 어휘 화해도 여기서 함께 한다.** `record-turn.ts:36`의 `ABSENT_GOAL_STATE`와
  `turn.ts`의 `unreadable_goal_state`가 **같은 사태를 두 어휘로 거부**하게 됐다. 두 경로를
  합치는 작업에서 반드시 화해시켜라 — 안 그러면 "goal_state를 못 읽었다"가 저장소에 두 개의
  다른 이름과 두 개의 다른 카운터로 존재한다.

**0단계 3번의 과업 목록 (충돌이 풀린 뒤)**
...
3. **G3** — `ABSENT_GOAL_STATE` ↔ `unreadable_goal_state` 어휘·카운터 화해(위 참조).
