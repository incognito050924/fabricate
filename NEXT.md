...
**조각 1 완료 (2026-07-25)** — 방어 게이트 다섯 개가 서 있고, 이 시점부터 모든 후속 작업이
기계에 감시된다. 작업 트리 깨끗.
...
**초록 증거 (2026-07-25 실측)**: `bun test` 58 pass / 0 fail (테스트 파일 5개, expect 78회),
`bunx tsc --noEmit` 통과, `bunx biome check .` 통과. 구현+테스트 합계 667줄, 게이트당 구현
52~76줄 — 옛 구현 실질 크기와 같은 자릿수.
...
3. `src/gate/evidence.ts` — 증거 종류 대조 (run→test·command / rescan→file /
   human→observation·repro, 증거 없으면 항상 차단)
4. `src/gate/red-first.ts` — 빨간 테스트 선행 검사 (루프 자작 무효, 빨강 관측 기록 필수,
   동결 해시 불일치·삭제 거부)
5. `src/gate/close.ts` — 종료 게이트 (잔여 있으면 done 부적격, 착지 상태는 게이트가 도출,
   미검증+재진입으로만 착지, 빈 조건 집합 완료 거부)
