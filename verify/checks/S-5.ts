...
const check: Check = {
  ...
  run: async (ctx) => {
    ...
    const run1 = hookEntries(observation.run1, "Stop");
    const run2 = hookEntries(observation.run2, "Stop");
    const entries = [...run1, ...run2];

    return {
      ok: run1.length > 0 && run2.length > 0,
      targets: entries.map((entry) => hookTarget("Stop", entry)),
      detail:
        run1.length > 0 && run2.length > 0
          ? ""
          : `Stop 훅이 부족하다: run1=${run1.length}, run2=${run2.length}`,
    };
  },
};
